...
int ctrl_add_input_tel(				/* return : 0 on success, other on error */
	...
	case InputCheck:
		...
		for(i = 0; i < ctrl->val.nbrows; i++)
		{
			...
			err = !strchr("03+", *val) ?  "il doit commencer par un 0 ou un + pour les numéros internationnaux" :
				(*val == '+' && val[1] == '0') ? "+0 n'est pas un préfixe autorisé" :
				(*val == '3' && c->len != 4) ? "les numéros courts doivent avoir 4 chiffres" :
				(*val == '0' && c->len != 10) ? "les numéros français doivent avoir 10 chiffres" :
				(c->len > 14) ? "il doit comporter moins de 14 caractères" :
				NULL;
			if(err)
			{
				ctrl->error |= 2;
				if(ctrl->errmsg) DYNBUF_ADD_STR(&ctrl->errmsg, "\n");
				DYNBUF_ADD3_CELL(&ctrl->errmsg, "Le numéro de téléphone [", &ctrl->val, i, 0, NO_CONV, "] n'est pas valide : ");
